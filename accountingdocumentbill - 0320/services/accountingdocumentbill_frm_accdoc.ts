...
export class AccDocService extends ListRepositoryService {
    ...
    amountCanEdit() {
        ...
        glAccDocEntryViewModel.dom.dataGrid_GLAccDocEntry.fields.forEach(ele => {
            ...
            ele['editable'] = (params: any) => {
                ...
                if ((dataItem['creditAmount'] > 0 && column.dataField === 'debitAmount')
                || (dataItem['debitAmount'] > 0 && column.dataField === 'creditAmount')) {
                    return false;
                }
                return true;
                // return false;
            ...
            if (change.type === ChangeType.Append || change.type === ChangeType.ValueChanged) {
                if (path[0] === 'numberOfAttch') {
                    if (change.value < 0 ) {
                        ...
                        //return this.bindingData.setValue(['numberOfAttch'], 0, true, true);
                    } else if (change.value > 9999) {
                        setTimeout(() => {
                            this.bindingData.setValue(['numberOfAttch'], 9999, true, true);
                        }, 0);
                        // return this.bindingData.setValue(['numberOfAttch'], 9999, true, true);
                    }
                    ...
                    if (Number(date) > Number(endDate) ) {
                        ...
                        //return this.bindingData.setValue(['accDocDateDisplay'], this.dateTo10(endDate), true, true);
                    //小于期间起始日期，赋起始日期
                    } else if (Number(date) < Number(beginDate)) {
                        setTimeout(() => {
                            this.bindingData.setValue(['accDocDateDisplay'], this.dateTo10(beginDate), true, true);
                        }, 0);
                        //return this.bindingData.setValue(['accDocDateDisplay'], this.dateTo10(beginDate), true, true);
                    }
                    //分录金额发生变化，进行合计
                    ...
                    //辅助金额发生变化，计算外币，并合计分录金额(汇率为0，不计算外币)
    ...
    saveAccDoc(): Observable<any> {
        ...
        const accDoc = this.befRepository.entityCollection.getEntityById(accDocID) as GLAccountingDocumentEntity;
        console.log(accDoc);
        this.befRepository.entityCollectionChange.publish();
        this.loadingService.show();
        return this.cardDataService.save()/* .catch((res: any) => {
            return this.catchError(res);
        }) */.pipe(
    ...
    assistanceAmount() {
        ...
        const amountTo2 = Number((exchangeRate * foreignCurrency).toFixed(2));
        if (amountTo2 !== amount) {
            const foreignCurrencyTo8 = Number((amount / foreignCurrency).toFixed(8));
            this.bindingData.setValue(['glAccDocEntrys', 'glAccDocAssistances', 'foreignCurrency'], foreignCurrencyTo8, true, true);
        }
    }
    ...
    total(accDocID: string): Observable<any> {
        ...
        for (let i = 0; i < accDocEntrys.count(); i++) {
            if (accDocEntrys[i].creditAmount === 0 && accDocEntrys[i].debitAmount !== 0) {
                setTimeout(() => {
                    accDocEntrys[i].lendingDirection = 'Debit';
                }, 0);
            } else if (accDocEntrys[i].creditAmount !== 0 && accDocEntrys[i].debitAmount === 0) {
                setTimeout(() => {
                    accDocEntrys[i].lendingDirection = 'Credit';
                }, 0);
            }
            if (accDocEntrys[i].lendingDirection === 'Credit') {
                sumJF = this.plus(sumJF, accDocEntrys[i].creditAmount);
            } else if (accDocEntrys[i].lendingDirection === 'Debit') {
                sumDF = this.plus(sumDF, accDocEntrys[i].debitAmount);
            }
        }
        this.frameContext.appContext.getFrameContext('glaccdocentry-component').uiState['TotalJF'] = sumJF; //Number(sumJF.toFixed(2));
        this.frameContext.appContext.getFrameContext('glaccdocentry-component').uiState['TotalDF'] = sumDF; //Number(sumDF.toFixed(2));
        this.frameContext.appContext.getFrameContext('glaccdocentry-component').uiState['Difference'] = sumJF - sumDF; //Number((sumJF - sumDF).toFixed(2));
        ...
        if (Math.abs(sumJF) >= 99999999999) {
            ...
            setTimeout(() => {
                this.bindingData.setValue(['glAccDocEntrys', 'creditAmount'], this.creditAmount0, true, true);
            }, 0);
        } else {
