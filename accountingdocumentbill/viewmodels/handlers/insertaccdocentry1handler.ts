...
@Injectable()
@NgCommandHandler({
  commandName: 'InsertAccDocEntry1'
})
export class InsertAccDocEntry1Handler extends CommandHandler {
  constructor(
  ...
  schedule() {
    ...
    this.addTask('insertAccDocEntry', (context: CommandContext) => {
      const args = [
        '{DATA~/basic-form-component/id}',
        '{DATA~/glaccdocentry-component/glAccDocEntrys/id}',
        ''
      ];
      return this.invoke(this._AccDocEntryService1, 'insertAccDocEntry', args, context);
    });
    ...
    this.addLink('insertAccDocEntry', 'entryAmount', `1==1`);
