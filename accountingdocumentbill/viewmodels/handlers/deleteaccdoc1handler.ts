...
@Injectable()
@NgCommandHandler({
  commandName: 'DeleteAccDoc1'
})
export class DeleteAccDoc1Handler extends CommandHandler {
  constructor(
  ...
  schedule() {
    ...
    this.addTask('deleteAccDoc', (context: CommandContext) => {
      const args = [
        '{UISTATE~/root-component/year/value}',
        '{COMMAND~/params/accDocID}',
        ''
      ];
      return this.invoke(this._AccDocService1, 'deleteAccDoc', args, context);
    });
    ...
    this.addLink('deleteAccDoc', 'entryAmount', `1==1`);
